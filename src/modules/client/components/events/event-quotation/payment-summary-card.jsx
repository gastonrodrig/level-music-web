...
export const PaymentSummaryCard = ({ quotationData, paymentType, onRegister, onCancel }) => {
  ...
  const amount = paymentType === "partial" ? quotationData.advancePayment : quotationData.totalAmount;
  ...
  return (
    <Box sx={{ p: 3, borderRadius: 3, bgcolor: colors.cardBg }}>
      ...
      <Box sx={{ mb: 2 }}>
        ...
        <Chip
          label={paymentType === "partial" ? "Pago Parcial (Anticipo)" : "Pago Total"}
          size="small"
          sx={{ mt: 0.5, bgcolor: paymentType === "partial" ? "#4caf50" : "#2196f3", color: "#fff" }}
        />
      </Box>
      ...
      <Box sx={{ mb: 3, display: "flex", alignItems: "center", gap: 1 }}>
        ...
        <Box>
          ...
          <Typography fontSize={14} sx={{ color: colors.textPrimary, fontWeight: 500 }}>
      ...
      <Stack spacing={2}>
        <Button
          variant="contained"
          onClick={onRegister}
          sx={{ py: 1, boxShadow: "none", textTransform: "none", fontWeight: 600, borderRadius: 2 }}
        >
          Registrar Pago
        </Button>
        <Button
