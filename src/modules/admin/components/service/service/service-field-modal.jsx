...
export const ServiceFieldModal = ({open, onClose, attributes, onAddAttribute, onAddCustom}) => {
  ...
  <Modal open={open} onClose={onClose}>
    <Box sx={{ p: 4, bgcolor: '#23232a', borderRadius: 3, maxWidth: 800, mx: 'auto', mt: '30vh' }}>
      ...
      {attributes?.map((attr, idx) => (
        <Box key={idx}  sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 1 ,bgcolor: '#000000ff', borderRadius:3, p:1}}>
          <IconButton
              color="primary"
      ...
      {/* Aquí puedes agregar la lógica para crear campos personalizados */}
      ...
      <Box display="flex" gap={1}>
          ...
          <Button
            ...
            disabled={!customName}
            onClick={() => {
              onAddAttribute({ name: customName, type: 'text', required: false });
              setCustomName('');
            }}
          >
            <Add />
