...
import { updateServiceModel } from '../../../../../shared/models/service/service/update-service-model';
...
export const ServiceEditPage = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const result = await startUpdateService(selected._id, updateServiceModel(data));
      if (result) {
    ...
    <Box component="form" sx={{ px: 4, pt: 2 }} onSubmit={handleSubmit(onSubmit)}>
      ...
      <Box sx={{ maxWidth: 1200, margin: "0 auto", mt: 2 }}>
        {details.map((detail, idx) => (
          <ServiceDetailBox
            key={detail._id || detail.service_detail_id || idx}
            index={idx}
            register={register}
            errors={errors}
            ...
            isEditMode
            setValue={setValue}
            // Pasa el detailId y el detailNumber correctos
